...
// by the minidriver in response to IOCTL_HID_GET_REPORT_DESCRIPTOR.  The first
// top-level collection is byte-for-byte Codex Micro (FF00/report ID 6).  The
// second FF70/report ID 7 collection is reserved for the native N4 bridge and
// is routed independently below.
//
HID_REPORT_DESCRIPTOR       G_DefaultReportDescriptor[] = {
    ...
    0xC0,                   // End primary Codex collection
...
--*/
{
    ...
    RtlZeroMemory(deviceContext->DeviceData, sizeof(deviceContext->DeviceData));
...

    {
        ...
        status = WdfWaitLockCreate(&lockAttributes, &deviceContext->BridgeLock);
        if (!NT_SUCCESS(status)) {
            KdPrint(("WdfWaitLockCreate failed 0x%x\\n", status));
            return status;
        }
    }

    // Retain the original custom-interface scaffold for low-level diagnosis.
    // Production user-mode traffic uses the FF70 companion HID collection;
    // ordinary custom IOCTL delivery through a HID minidriver is not assumed.
    status = WdfDeviceCreateDeviceInterface(
                            device,
                            &GUID_DEVINTERFACE_MIRABOX_CODEX_MICRO_BRIDGE,
                            NULL);
    if (!NT_SUCCESS(status)) {
    ...
    status = BridgeQueueCreate(device,
                               &deviceContext->BridgeOutputQueue);
    if (!NT_SUCCESS(status)) {
        return status;
    }
...
--*/
{
    ...
    RtlZeroMemory(queueContext->OutputReport, sizeof(queueContext->OutputReport));
...
--*/
{
    ...
    {
    case IOCTL_MIRABOX_CODEX_PUSH_INPUT:
        status = BridgePushInput(deviceContext, Request);
        break;

    case IOCTL_MIRABOX_CODEX_READ_OUTPUT:
        status = BridgeReadOutput(deviceContext, Request, &completeRequest);
        break;

    case IOCTL_MIRABOX_CODEX_GET_INFO:
        status = BridgeGetInfo(deviceContext, Request);
        break;

    case IOCTL_MIRABOX_CODEX_RESET:
        status = BridgeReset(deviceContext, Request);
        break;
...
--*/
{
    ...
    // IOCTL_HID_READ_REPORT has no input report-ID selector.  The HID class
    // consumes one common pending read and dispatches the returned report to
    // the matching top-level collection using its first-byte report ID.
    status = BridgeReadInput(
        QueueContext->DeviceContext, Request, CompleteRequest);
    if (!NT_SUCCESS(status) && *CompleteRequest) {
        KdPrint(("BridgeReadInput failed with 0x%x\n", status));
    }
    return status;
}
...

{
    ...
    reportSize = MIRABOX_CODEX_MICRO_REPORT_LENGTH;
    if (packet.reportId == CONTROL_COLLECTION_REPORT_ID) {
        status = BridgeCopyHidPacketForId(
            &packet, CONTROL_COLLECTION_REPORT_ID, report);
        if (!NT_SUCCESS(status)) {
            return status;
        }

        // Preserve the payload for diagnostics and make it available to the
        // native bridge process as a queued Codex output report.
        RtlCopyMemory(QueueContext->DeviceContext->DeviceData,
                      report + 1,
                      sizeof(QueueContext->DeviceContext->DeviceData));
        status = BridgePublishOutput(QueueContext->DeviceContext, report);
    } else if (packet.reportId == COMPANION_COLLECTION_REPORT_ID) {
        UCHAR companionReport[MIRABOX_CODEX_MICRO_REPORT_LENGTH];

        status = BridgeCopyHidPacketForId(
            &packet, COMPANION_COLLECTION_REPORT_ID, companionReport);
        if (!NT_SUCCESS(status)) {
            return status;
        }
        // Translate the companion collection's opaque body into the primary
        // Codex report and feed the same input path Codex uses.
        companionReport[0] = MIRABOX_CODEX_MICRO_REPORT_ID;
        status = BridgePushInputReport(
            QueueContext->DeviceContext, companionReport);
    } else {
...
--*/
{
    ...
    if (packet.reportId == CONTROL_COLLECTION_REPORT_ID ||
        packet.reportId == COMPANION_COLLECTION_REPORT_ID) {
        if (!BridgeTryPopInputForId(
                QueueContext->DeviceContext, packet.reportId, report)) {
            RtlZeroMemory(report, sizeof(report));
            report[0] = packet.reportId;
            // Match the Codex/OpenMicro control GET_REPORT response: an empty
            // frame still identifies RPC channel 2 and carries a zero payload.
    ...
    return RequestCopyFromBuffer(Request, report, reportSize);
}
...
--*/
{
    ...
    if (packet.reportId == CONTROL_COLLECTION_REPORT_ID) {
        ...
        RtlCopyMemory(QueueContext->OutputReport,
                      report + 1,
                      sizeof(QueueContext->OutputReport));
        status = BridgePublishOutput(QueueContext->DeviceContext, report);
    } else if (packet.reportId == COMPANION_COLLECTION_REPORT_ID) {
        status = BridgeCopyHidPacketForId(
            &packet, COMPANION_COLLECTION_REPORT_ID, report);
        if (!NT_SUCCESS(status)) {
            return status;
        }
        report[0] = MIRABOX_CODEX_MICRO_REPORT_ID;
        status = BridgePushInputReport(QueueContext->DeviceContext, report);
    } else {
        status = STATUS_INVALID_PARAMETER;
...
--*/
{
    ...
    // The original sample starts a periodic synthetic report timer. In this
    // bridge the native N4 process is the sole input producer; synthetic
    // timer traffic would look like garbage JSON to Codex, so leave it
    // stopped. EvtTimerFunc remains in the sample for easy local experiments.
...
--*/
{
    ...
    if (NT_SUCCESS(status)) {
        ...
        RtlCopyMemory(readReport.Data,
                      queueContext->DeviceContext->DeviceData,
                      sizeof(readReport.Data));
