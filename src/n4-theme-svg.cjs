...
function infoStripGroup(key,x,y){
  ...
  if(info.index===0&&info.clock){
    ...
    const hint=info.enabled===false?'旋钮 1 · 已停用':`旋钮 1 · ${title}`;
  ...
  return `<g class="n4-key" data-strip-info="true"><rect x="${x+1}" y="${y+1}" width="174" height="109" rx="8" fill="#111a27" stroke="#2e3b4c"/><text x="${x+12}" y="${y+19}" fill="#7e8b9e" font-size="11" font-family="Microsoft YaHei,sans-serif">旋钮 ${info.index+1}</text><image href="${imageData(catalog.knobIcons[mode])}" x="${x+12}" y="${y+39}" width="25" height="25"/><text x="${x+47}" y="${y+58}" fill="#f4f1e8" font-size="17" font-weight="700" font-family="Microsoft YaHei,sans-serif">${escape(title)}</text><text x="${x+(info.fastTouch?62:88)}" y="${y+94}" text-anchor="middle" fill="#7e8b9e" font-size="12" font-family="Microsoft YaHei,sans-serif">${escape(info.fastTouch?(info.brightness==null?'旋转调亮度':info.brightness+'%'):detail)}</text>${info.fastTouch?`<path data-fast-touch-divider="true" d="M${x+116} ${y+16}V${y+96}" stroke="#253140"/><g data-fast-touch="true"><title>点击最右触摸区：加速模式</title><rect x="${x+124}" y="${y+34}" width="44" height="44" rx="7" fill="#342b18" stroke="#edbb46"/><path d="M${x+148} ${y+38}L${x+135} ${y+57}H${x+145}L${x+140} ${y+73}L${x+159} ${y+50}H${x+149}Z" fill="#ffd25a"/></g>`:''}</g>`;
}
