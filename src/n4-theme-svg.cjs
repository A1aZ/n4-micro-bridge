...
function infoStripGroup(key,x,y){
  ...
  if(info.index===0&&info.clock){
    const weekday=info.clock.weekday?` · ${info.clock.weekday}`:'';
    const hint=info.enabled===false?'旋钮 1 · 已停用':title?`旋钮 1 · ${title}`:'旋钮 1';
    return `<g class="n4-key" data-strip-info="true" data-strip-clock="true"><rect x="${x+1}" y="${y+1}" width="174" height="109" rx="8" fill="#111a27" stroke="#2e3b4c"/><text x="${x+12}" y="${y+19}" fill="#7e8b9e" font-size="11" font-family="Microsoft YaHei,sans-serif">当前时间</text><text x="${x+164}" y="${y+19}" text-anchor="end" fill="#7e8b9e" font-size="10" font-family="Microsoft YaHei,sans-serif">${escape(info.clock.date+weekday)}</text><text x="${x+12}" y="${y+61}" fill="#f4f1e8" font-size="35" font-weight="700" font-family="Bahnschrift Condensed,Bahnschrift,Segoe UI,sans-serif">${escape(info.clock.time)}</text><image href="${imageData(catalog.knobIcons[mode])}" x="${x+12}" y="${y+80}" width="18" height="18"/><text x="${x+38}" y="${y+94}" fill="#7e8b9e" font-size="11" font-family="Microsoft YaHei,sans-serif">${escape(hint)}</text></g>`;
  }
  if(touchAction){
    const splitDetail=info.enabled===false?'已停用':({micro:'Codex 控制',scroll:'聊天区',reasoning:info.reasoningConfigured?'− / +':'待绑定',brightness:info.brightness==null?'每格 5%':info.brightness+'%'})[mode];
    const target=touchAction.targetKey||'ACT06',eyebrow=touchAction.eyebrow||'快捷',label=touchAction.label||'加速',scope=touchAction.scope||'strip';
    const knobTitle=title?`<text x="${x+42}" y="${y+54}" fill="#f4f1e8" font-size="15" font-weight="700" font-family="Microsoft YaHei,sans-serif">${escape(title)}</text>`:'';
    return `<g class="n4-key" data-strip-info="true" data-strip-layout="split-action"><g data-knob-panel="true"><rect x="${x+1}" y="${y+1}" width="104" height="109" rx="8" fill="#111a27" stroke="#2e3b4c"/><text x="${x+10}" y="${y+19}" fill="#7e8b9e" font-size="11" font-family="Microsoft YaHei,sans-serif">旋钮 ${info.index+1}</text><image href="${imageData(catalog.knobIcons[mode])}" x="${x+10}" y="${y+35}" width="24" height="24"/>${knobTitle}<text x="${x+53}" y="${y+94}" text-anchor="middle" fill="#7e8b9e" font-size="12" font-family="Microsoft YaHei,sans-serif">${escape(splitDetail)}</text></g><g data-touch-action="${escape(target)}" data-touch-scope="${escape(scope)}"><title>触摸整块第 4 区：${escape(label)}</title><rect x="${x+113}" y="${y+1}" width="61" height="109" rx="8" fill="#231e14" stroke="#7c6228"/><text x="${x+144}" y="${y+19}" text-anchor="middle" fill="#b8a36c" font-size="10" font-family="Microsoft YaHei,sans-serif">${escape(eyebrow)}</text><path d="M${x+146} ${y+29}L${x+128} ${y+58}H${x+140}L${x+134} ${y+83}L${x+161} ${y+49}H${x+148}Z" fill="#ffd25a"/><text x="${x+144}" y="${y+99}" text-anchor="middle" fill="#f4f1e8" font-size="12" font-weight="700" font-family="Microsoft YaHei,sans-serif">${escape(label)}</text></g></g>`;
  }
  const titleMarkup=title?`<text x="${x+47}" y="${y+58}" fill="#f4f1e8" font-size="17" font-weight="700" font-family="Microsoft YaHei,sans-serif">${escape(title)}</text>`:'';
  return `<g class="n4-key" data-strip-info="true"><rect x="${x+1}" y="${y+1}" width="174" height="109" rx="8" fill="#111a27" stroke="#2e3b4c"/><text x="${x+12}" y="${y+19}" fill="#7e8b9e" font-size="11" font-family="Microsoft YaHei,sans-serif">旋钮 ${info.index+1}</text><image href="${imageData(catalog.knobIcons[mode])}" x="${x+12}" y="${y+39}" width="25" height="25"/>${titleMarkup}<text x="${x+88}" y="${y+94}" text-anchor="middle" fill="#7e8b9e" font-size="12" font-family="Microsoft YaHei,sans-serif">${escape(detail)}</text></g>`;
}
